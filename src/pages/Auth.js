...
function Auth() {
    ...
    useEffect(() => {
        ...
        if (r) {
            ...
            try {
                localStorage.setItem("fip_ref", r);
            }
            catch { /**/ }
        }
        else {
    ...
    async function fastRedirect(user) {
        ...
        if (!pinSetup) {
            try {
                ...
                if (!st?.hasPin) {
                    setPinSetup({ id: userId, email: user?.email });
                    return;
                }
    ...
    async function submit(e) {
        ...
        if (mode === "signup") {
            ...
            if (!phone.trim()) {
                setFormError("Renseignez votre numéro de téléphone.");
                return;
            }
        }
        setLoading(true);
        try {
            if (mode === "signup") {
                ...
                if (error)
                    throw error;
                try {
                    localStorage.removeItem("fip_ref");
                }
                catch { /**/ }
    ...
    async function verifyOTP(e) {
        ...
        try {
            ...
            if (apiErr || res?.error)
                throw new Error(apiErr?.message || res?.error || "Code invalide");
    ...
    if (pinSetup) {
        return (<PinLock mode="create" onSuccess={() => {
        ...
        <div className="w-full max-w-sm">
          ...
          {mode === "2fa" || mode === "registration_otp" ? (<form className="mt-8 space-y-4" onSubmit={verifyOTP}>
              <div className="flex flex-col items-center gap-4">
                ...
                {formError && (<div role="alert" className="flex items-start gap-2 rounded-2xl border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive w-full">
                    <AlertCircle className="mt-0.5 h-4 w-4 shrink-0"/>
                    <span>{formError}</span>
                  </div>)}
                <input required type="text" maxLength={6} value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, ""))} placeholder="000000" className="w-full rounded-full border border-border bg-surface-2 py-4 text-center text-3xl font-bold tracking-[0.5em] outline-none focus:border-primary focus:ring-2 focus:ring-primary/30"/>
                <button type="submit" disabled={loading || otp.length < 6} className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-gradient-primary py-3 text-sm font-semibold text-primary-foreground shadow-glow transition-transform hover:scale-[1.02] disabled:opacity-60">
                  {loading ? <Loader2 className="h-4 w-4 animate-spin"/> : (<>Vérifier le code <CheckCircle2 className="h-4 w-4"/></>)}
                </button>
                <button type="button" onClick={() => { setMode("login"); supabase.auth.signOut(); }} className="text-xs text-muted-foreground hover:underline">
                  {mode === "registration_otp" ? "Retour à l'inscription" : "Annuler et se reconnecter"}
                </button>
              </div>
            </form>) : (<form className="mt-8 space-y-4" onSubmit={submit}>
            {formError && (<div role="alert" className="flex items-start gap-2 rounded-2xl border border-destructive/40 bg-destructive/10 p-3 text-sm text-destructive">
                <AlertCircle className="mt-0.5 h-4 w-4 shrink-0"/>
                <span>{formError}</span>
              </div>)}
            {mode === "signup" && (<>
                {refCode && (<div className="rounded-2xl border border-primary/30 bg-primary/5 p-3 text-xs text-primary">
                ...
                <div className="relative">
                  ...
                  <input required value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Nom complet" className="w-full rounded-full border border-border bg-surface-2 py-3 pl-10 pr-4 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/30"/>
                </div>
                <div className="relative">
                  <Phone className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground"/>
                  <input required value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+226 70 00 00 00" className="w-full rounded-full border border-border bg-surface-2 py-3 pl-10 pr-4 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/30"/>
                </div>
              </>)}
            <div className="relative">
              <Mail className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground"/>
              <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="vous@exemple.com" className="w-full rounded-full border border-border bg-surface-2 py-3 pl-10 pr-4 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/30"/>
            </div>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground"/>
              <input required type="password" minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Mot de passe" className="w-full rounded-full border border-border bg-surface-2 py-3 pl-10 pr-4 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/30"/>
            </div>
            <button type="submit" disabled={loading} className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-gradient-primary py-3 text-sm font-semibold text-primary-foreground shadow-glow transition-transform hover:scale-[1.02] disabled:opacity-60">
