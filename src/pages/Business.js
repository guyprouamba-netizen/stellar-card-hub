...
import { listMyBusinesses, createBusiness, listPaymentLinks, createPaymentLink, updatePaymentLink, listLinkPayments, listApiKeys, createApiKey, revokeApiKey, cashoutBusinessBalance, listProjects, createProject, getBusinessDashboard, updateBusiness, updateProject, listShopTemplates, applyShopTemplate, } from "@/lib/business.functions";
import { listOrders, updateOrderStatus, listBusinessPosts, createBusinessPost, updateBusinessPost, deleteBusinessPost, } from "@/lib/orders.functions";
...
import { ArrowLeft, Building2, Copy, Link2, Plus, Wallet, FolderKanban, TrendingUp, TrendingDown, Sparkles, Store, Package, Megaphone, ExternalLink, Eye, Palette, Loader2, MessageSquare, Activity } from "lucide-react";
...
export default function BusinessPage() {
    ...
    const [postDraft, setPostDraft] = useState({ title: "", body: "", image_url: "" });
    ...
    async function refreshCurrent(bizId) {
        // Un onglet en erreur ne doit pas casser les autres — chaque appel a son propre fallback.
        const safe = (p, fb) => p.catch((e) => { console.warn("[business fetch]", e?.message); return fb; });
        const [l, p, k, pr, d, o, po] = await Promise.all([
            safe(listPaymentLinks(bizId), []),
            safe(listLinkPayments(bizId), []),
            safe(listApiKeys(bizId), []),
            safe(listProjects(bizId), []),
            safe(getBusinessDashboard(bizId), null),
            safe(listOrders(bizId), []),
            safe(listBusinessPosts(bizId), []),
        ]);
        setLinks(l);
        setPayments(p);
        setKeys(k);
    ...
    useEffect(() => { if (session)
        refreshAll(); /* eslint-disable-next-line */ }, [session]);
    useEffect(() => { if (current)
        refreshCurrent(current.id); }, [current?.id]);
    ...
    async function onEditLink(l) {
        const title = prompt("Titre du lien", l.title);
        if (title === null)
            return;
        const amountStr = prompt("Montant fixe en " + l.currency + " (vide = paiement flexible)", l.amount !== null && l.amount !== undefined ? String(l.amount) : "");
        if (amountStr === null)
            return;
    ...
    async function onCreateKey() {
        if (!current)
            return;
        try {
            const k = await createApiKey({ business_id: current.id, label: "default", mode: "live" });
            setNewKey(k.api_key);
    ...
    async function onRevokeKey(id) {
        if (!confirm("Révoquer cette clé API ?"))
            return;
        try {
    ...
    async function onCashout() {
        if (!current)
            return;
        try {
            const r = await cashoutBusinessBalance(current.id);
            if (r.ok) {
                toast.success(`${r.transferred.toLocaleString("fr-FR")} XOF transférés vers votre wallet ✅`);
                refreshAll();
            }
            else
                toast.error(r.error);
    ...
    function copy(text) { navigator.clipboard.writeText(text); toast.success("Copié"); }
    async function onOrderStatus(o, next) {
        try {
            const u = await updateOrderStatus({ id: o.id, status: next });
            setOrders((prev) => prev.map((x) => x.id === o.id ? { ...x, ...u } : x));
    ...
    async function onUploadPostImage(file) {
        setUploadingImg(true);
        try {
            const url = await uploadBusinessMedia(file, "posts");
            setPostDraft((d) => ({ ...d, image_url: url }));
        }
        catch (e) {
            toast.error(e.message);
        }
    ...
    async function onCreatePost(publish) {
        if (!current || !postDraft.title.trim()) {
            toast.error("Titre requis");
            return;
        }
        try {
            const p = await createBusinessPost({
    ...
    async function onTogglePost(p) {
        try {
            const u = await updateBusinessPost({ id: p.id, published: !p.published });
            setPosts((prev) => prev.map((x) => x.id === p.id ? { ...x, ...u } : x));
        }
        catch (e) {
            toast.error(e.message);
        }
    ...
    const lightLabel = { green: "🟢 En croissance", yellow: "🟡 Stable", red: "🔴 En baisse" };
    if (loading)
    ...
    return (<div className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-30 border-b border-border bg-surface-1/80 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            {businesses.length > 1 && (<select value={current?.id || ""} onChange={(e) => setCurrent(businesses.find((b) => b.id === e.target.value) || null)} className="rounded-xl border border-border bg-surface-2 px-3 py-2 text-xs font-semibold outline-none">
      ...
      <main className="mx-auto max-w-7xl px-3 py-4 sm:px-6 sm:py-6">
        {businesses.length === 0 ? (<div className="rounded-3xl border border-border bg-card p-12 text-center">
            ...
            <aside className="hidden w-60 shrink-0 lg:block">
              <nav className="sticky top-20 space-y-1 rounded-2xl border border-border bg-card p-3">
                ...
                return (<button key={n.id} onClick={() => setTab(n.id)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition ${active ? "bg-primary/15 text-primary" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}>
                      <Icon className="h-4 w-4"/> {n.label}
                    </button>);
            })}
                {current && (<div className="mt-3 space-y-1 border-t border-border pt-3">
            ...
            <div className="-mx-3 mb-4 flex gap-2 overflow-x-auto px-3 pb-1 lg:hidden">
              {NAV.map((n) => (<button key={n.id} onClick={() => setTab(n.id)} className={`shrink-0 rounded-full border px-3 py-1.5 text-xs font-semibold ${tab === n.id ? "border-primary bg-primary/10 text-primary" : "border-border bg-surface-2 text-muted-foreground"}`}>
                  {n.label}
                </button>))}
            </div>
            ...
            {current && (<div className="space-y-6">
              {tab === "overview" && (<>
                ...
                <div className="mb-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                  {[
                    ...
                    ].map((s) => (<div key={s.label} className="rounded-2xl border border-border bg-card px-4 py-3">
                ...
                <div className="rounded-3xl border border-border bg-card p-4 shadow-card-premium sm:p-6">
                  <div className="flex flex-wrap items-start justify-between gap-4">
                    ...
                    <div className="text-left sm:text-right">
                      ...
                      <button onClick={onCashout} disabled={Number(current.balance) <= 0} className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-foreground px-3 py-1.5 text-xs font-semibold text-background disabled:opacity-40 transition-all hover:scale-105 active:scale-95">
                        <Wallet className="h-3.5 w-3.5"/> Transférer vers mon wallet
                      </button>
                    </div>
                  </div>
                  {dash && (<div className={`mt-6 grid gap-3 rounded-2xl border bg-gradient-to-br p-5 sm:grid-cols-3 ${lightColors[dash.kpis.light]}`}>
                      <div>
              ...
              {tab === "settings" && (<>
                <ShopTemplateSelector business={current} onUpdated={refreshAll}/>
                ...
                {tab === "products" && (<ProductsPanel businessId={current.id} shopSlug={current.slug}/>)}

                {tab === "sms" && (<div className="space-y-6">
                ...
                {tab === "projects" && (<section>
                  ...
                  {projects.length === 0 ? (<div className="rounded-2xl border border-dashed border-border bg-surface-2 p-8 text-center">
                    ...
                    </div>) : (<div className="grid gap-6 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2">
                      {projects.map((p) => {
                            return (<div key={p.id} className="group relative flex flex-col overflow-hidden rounded-[2rem] border border-border bg-card transition-all hover:border-primary/40 hover:shadow-card-premium min-h-[400px]">
                             {/* Badge de Type */}
                             ...
                             <div className="p-8 pb-0">
                               <div className="flex items-start justify-between">
                                 <div className="flex flex-1 items-center gap-5">
                                   {p.logo_url ? (<img src={p.logo_url} className="h-20 w-20 rounded-[1.5rem] object-cover shadow-md border border-border/50" alt=""/>) : (<div className="grid h-20 w-20 place-items-center rounded-[1.5rem] bg-gradient-to-br from-primary/20 to-primary/5 text-3xl font-black text-primary border border-primary/10">
                                       {p.name[0]}
                                     </div>)}
                                   <div className="min-w-0 flex-1">
                                     <div className="flex items-center gap-2">
                ...
                {tab === "links" && (<section>
                  ...
                  <div className="space-y-2">
                    ...
                    {links.map((l) => {
                        ...
                        return (<div key={l.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-surface-2 p-4">
                          <div className="min-w-0 flex-1">
                  ...
                  <div className="-mx-3 overflow-x-auto rounded-2xl border border-border bg-surface-2 sm:mx-0">
                    <table className="w-full min-w-[640px] text-sm">
                      ...
                      <tbody>
                        ...
                        {payments.map((p) => (<tr key={p.id} className="border-t border-border">
                ...
                {tab === "orders" && (<section>
                  ...
                  {orders.length === 0 ? (<div className="rounded-2xl border border-dashed border-border bg-surface-2 p-8 text-center">
                      ...
                      {orders.map((o) => (<div key={o.id} className="rounded-2xl border border-border bg-surface-2 p-4">
                          ...
                          <div className="mt-3 grid grid-cols-2 gap-1 border-t border-border pt-3 text-xs sm:grid-cols-3">
                            {(o.items || []).map((it, i) => (<div key={i} className="text-muted-foreground"><span className="font-semibold text-foreground">{it.name}</span> × {it.quantity}</div>))}
                          </div>
      ...
      {configProject && (<ProjectConfigSheet project={configProject} onClose={() => setConfigProject(null)} onSaved={() => { if (current)
...
function ShopBrandingPanel({ biz, onUpdated }) {
    ...
    useEffect(() => {
        ...
        setTheme({ ...SHOP_THEME_DEFAULT, ...(biz.theme || {}) });
        setLogoUrl(biz.logo_url || "");
        setCoverUrl(biz.cover_url || "");
    }, [biz.id]);
    async function onFile(kind, f) {
        setUploading(kind);
        try {
            ...
            if (kind === "logo")
                ...
                setCoverUrl(url);
            await updateBusiness({ id: biz.id, [kind === "logo" ? "logo_url" : "cover_url"]: url });
    ...
    async function saveText() {
        setSaving(true);
        try {
            await updateBusiness({ id: biz.id, name, description, tagline, theme });
    ...
    return (<section className="mt-6 rounded-3xl border border-border bg-card p-4 shadow-card-premium sm:p-6">
      <div className="flex items-center justify-between">
        ...
        <div>
          ...
          <div className="mt-2 flex items-center gap-3">
            ...
            <label className="cursor-pointer rounded-full border border-border bg-surface-2 px-3 py-1.5 text-xs font-semibold hover:bg-muted">
              ...
              <input type="file" accept="image/*" className="hidden" disabled={uploading === "logo"} onChange={(e) => e.target.files?.[0] && onFile("logo", e.target.files[0])}/>
            </label>
          </div>
        ...
        <div>
          ...
          <div className="mt-2 flex items-center gap-3">
            ...
            <label className="cursor-pointer rounded-full border border-border bg-surface-2 px-3 py-1.5 text-xs font-semibold hover:bg-muted">
              ...
              <input type="file" accept="image/*" className="hidden" disabled={uploading === "cover"} onChange={(e) => e.target.files?.[0] && onFile("cover", e.target.files[0])}/>
            </label>
          </div>
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nom de la boutique" className="rounded-lg border border-border bg-background px-3 py-2 text-sm"/>
        <input value={tagline} onChange={(e) => setTagline(e.target.value)} placeholder="Slogan (affiché sur la couverture)" className="rounded-lg border border-border bg-background px-3 py-2 text-sm"/>
        <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Description courte (affichée sur la vitrine et les reçus)" rows={3} className="md:col-span-2 rounded-lg border border-border bg-background px-3 py-2 text-sm"/>
      </div>
      ...
      <div className="mt-6 border-t border-border pt-5">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {SHOP_PRESETS.map((p) => (<button key={p.label} onClick={() => setTheme(p.theme)} className="flex items-center gap-2 rounded-full border border-border bg-surface-2 px-3 py-1.5 text-xs font-semibold hover:bg-muted">
              <span className="flex">
                {[p.theme.bg, p.theme.surface, p.theme.primary].map((c) => (<span key={c} className="h-4 w-4 rounded-full border border-border -ml-1 first:ml-0" style={{ background: c }}/>))}
              </span>
              {p.label}
            </button>))}
        </div>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {THEME_FIELDS.map((f) => (<label key={f.key} className="flex items-center gap-3 rounded-xl border border-border bg-surface-2 px-3 py-2">
              <input type="color" value={theme[f.key]} onChange={(e) => setTheme((t) => ({ ...t, [f.key]: e.target.value }))} className="h-8 w-10 cursor-pointer rounded border border-border bg-transparent"/>
      ...
      <button onClick={saveText} disabled={saving} className="mt-5 w-full rounded-full bg-gradient-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground shadow-glow disabled:opacity-50 sm:w-auto sm:px-8">
...
// ============================================================
function ShopProjectsPanel({ projects, onChanged, onGoProjects }) {
    const [busy, setBusy] = useState(null);
    async function toggle(p) {
        setBusy(p.id);
        try {
            await updateProject({ id: p.id, show_in_shop: !p.show_in_shop });
            toast.success(!p.show_in_shop ? "Projet ajouté à la boutique ✅" : "Projet retiré de la boutique");
            onChanged();
        }
        catch (e) {
            toast.error(e.message);
    ...
    return (<section className="mt-6 rounded-3xl border border-border bg-card p-4 shadow-card-premium sm:p-6">
      ...
      {projects.length === 0 ? (<div className="mt-4 rounded-2xl border border-dashed border-border bg-surface-2 p-6 text-center">
          ...
          {projects.map((p) => {
                const on = Boolean(p.show_in_shop);
                return (<div key={p.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-surface-2 px-4 py-3">
                ...
                <button onClick={() => toggle(p)} disabled={busy === p.id} className={`rounded-full px-4 py-1.5 text-xs font-semibold transition ${on ? "bg-emerald-500/15 text-emerald-500" : "border border-border hover:bg-muted"}`}>
...
// ============================================================
function ShopTemplateSelector({ business, onUpdated }) {
    const [templates, setTemplates] = useState([]);
    const [loading, setLoading] = useState(true);
    const [busy, setBusy] = useState(null);
    useEffect(() => {
        listShopTemplates().then((r) => {
            setTemplates(r.templates || []);
            setLoading(false);
        }).catch(() => setLoading(false));
    }, []);
    async function onApply(tid) {
        setBusy(tid || "reset");
        try {
    ...
    return (<section className="mt-6 rounded-3xl border border-border bg-card p-4 shadow-card-premium sm:p-6">
      ...
      {loading ? <div className="py-10 text-center"><Loader2 className="h-6 w-6 animate-spin mx-auto text-muted-foreground"/></div> : (<div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <div className={`overflow-hidden rounded-2xl border transition ${!business.template_id ? 'border-primary ring-2 ring-primary/20' : 'border-border'}`}>
            ...
            <div className="p-4">
              ...
              <button onClick={() => onApply(null)} disabled={!business.template_id || busy === "reset"} className="mt-4 w-full rounded-full border border-border py-1.5 text-xs font-semibold hover:bg-muted disabled:opacity-50">
          ...
          {templates.map((t) => {
                ...
                return (<div key={t.id} className={`overflow-hidden rounded-2xl border transition ${active ? 'border-primary ring-2 ring-primary/20' : 'border-border'}`}>
                ...
                <div className="p-4">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <button onClick={() => onApply(t.id)} disabled={active || !!busy} className={`flex-1 rounded-full py-1.5 text-xs font-semibold transition ${active ? 'bg-primary/10 text-primary' : 'border border-border hover:bg-muted'} disabled:opacity-50`}>
                      {busy === t.id ? "..." : active ? "Appliquer" : "Appliquer"}
                    </button>
                    <button onClick={() => window.open(`/shop/demo?template_id=${t.id}&biz_id=${business.id}`, '_blank')} className="grid h-8 w-8 place-items-center rounded-full border border-border hover:bg-muted" title="Prévisualiser">
